...
def retrieve_unmapped_reads(args, params, filenames):
    ...
    try:
        ...
        if not args.b is None:
            pysam.fastq('-@', '%d' % thread_n, '-f', '12', '-F', '256', '-N', '-0', '/dev/null', '-1', filenames.unmapped_1, '-2', filenames.unmapped_2, '-s', '/dev/null', args.b)
        elif not args.c is None:
            pysam.fastq('-@', '%d' % thread_n, '-f', '12', '-F', '256', '-N', '-0', '/dev/null', '-1', filenames.unmapped_1, '-2', filenames.unmapped_2, '-s', '/dev/null', '--reference', args.fa, args.c)
